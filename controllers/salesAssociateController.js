...
async function create(req, res) {
  try {
    ...
    if(!name || !username || !password || !email_addr) {
      return res.status(400).json({success: false, message: 'name, username, password, and emaill address are needed for new sales associate'});
    }

    //hash the password before storing in database
    const user_pass = await bcrypt.hash(password, SALT_ROUNDS);
    const associate = await salesModel.create({name, username, user_pass, email_addr, address});
