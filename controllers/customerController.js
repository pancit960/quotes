...
async function getAll(req, res) {
    ...
    //debug for group
        ...
        res.status(500).json({success: false, message: 'Could not retrieve all customers from external legacy database. FIX THIS!'});
    }
}
...
async function getSingle(req, res) {
    try {
        ...
        if(!customer) {
            ...
            return res.status(404).json({success: false, message: 'Could not find customer.'});
        }
    }
    catch(err) {
        console.error('[customerController.getSingle]', err);
        res.status(500).json({success: false, nessage: 'Function failed to search a customer.'});
    }
}
...
async function search(req, res) {
    try {
        ...
        res.status(500).json({success: false, message: 'Function failed to search a customer'});
    }
}
