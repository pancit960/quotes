...
async function findById(id) {
  const rows = await query(
    `SELECT id, name, username, email_addr, address, commission 
        FROM associate_data 
        WHERE id = ?`,
    [id],
  );
...
async function create({ id, name, username, user_pass, email_addr, address }) {
  await query(
    `INSERT INTO associate_data (name, username, user_pass, email_addr, address) 
        VALUES (?, ?, ?, ?, ?)`,
...
async function update(id, data) {
  ...
  await query(
    `UPDATE associate_data SET ${fields.join(", ")} WHERE id = ?`,
    values,
  );
...
async function addCommission(id, amount) {
  await query(
    `UPDATE associate_data SET commission = commission + ? 
        WHERE ID = ?`,
    [amount, id],
  );
  return findById(id);
