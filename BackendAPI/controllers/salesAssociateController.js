...
async function getAll(req, res) {
  try {
    ...
    res.status(500).send("Failed to get a sales associate");
  }
}
  ...
  try {
    ...
    if (!associate) {
      return res.status(404).json({
        success: false,
        message: "Sales associate could not be found, try again.",
      });
    }
    //found associate
    ...
    res.status(500).json({
      success: false,
      message: "Could not search for a sales associate, needs fix",
    });
  }
}
...
async function create(req, res) {
  try {
    ...
    if (!name || !username || !password || !email_addr) {
      return res.status(400).json({
        success: false,
        message:
          "name, username, password, and emaill address are needed for new sales associate",
      });
    }

    //hash the password before storing in database
    const user_pass = await bcrypt.hash(password, SALT_ROUNDS);
    const associate = await salesModel.create({
      name,
      username,
      user_pass,
      email_addr,
    ...
    if (err.code === "ER_DUP_ENTRY") {
      return res
        .status(409)
        .json({ success: false, message: "Associate already exists!" });
    }
    //failure, need to fix somewhere
    console.error("[salesAssociateController.create]", err);
    res.status(500).json({
      success: false,
      message: "Could not create this associate - error",
    });
  }
}
...
async function update(req, res) {
  try {
    ...
    if (!associate) {
      return res
        .status(404)
        .json({ success: false, message: "Associate could not be found" });
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "Could not update sales associate" });
  }
}
...
async function remove(req, res) {
  try {
    ...
    if (!deleted) {
      return res
        .status(404)
        .json({ success: false, message: "Could not find sales associate" });
    }
    res.redirect("/admin/associates");
  } catch (err) {
    ...
    res
      .status(500)
      .json({ success: false, message: "Could not delete associate - error" });
  }
}
